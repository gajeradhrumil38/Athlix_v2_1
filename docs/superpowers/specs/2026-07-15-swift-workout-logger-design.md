...
## Known Limitations (found during the per-task review cycle and a final holistic pass, consciously deferred rather than fixed in this milestone)

All 20 planned tasks landed, were independently spec- and code-quality-reviewed, and a Critical bug (a duplicate-write risk in `WorkoutRepository.saveWorkout` when the RPC succeeds but the post-RPC fetch-back fails) was found and fixed during that cycle — see commit `8c01225`. The items below are real, identified gaps that were deliberately not fixed in this pass, tracked here as explicit debt rather than silently shipped:

- **`ExerciseTypeResolver.normalizeKey` doesn't collapse all whitespace types.** It only collapses repeated ASCII spaces (`while result.contains("  ")`), while the TS source's `.replace(/\s+/g, ' ')` collapses any whitespace run (tabs, newlines) into one space. Low real-world risk since exercise names come from a curated catalog/UI, not free-form multi-line text, but a genuine deviation from the ground truth worth closing before this resolver fans out to more call sites.
- **`Live*Repository` network-calling code has no executable-test coverage at the `Live` implementation level.** This codebase's established convention (present before this milestone, followed throughout it) is to test repositories via hand-written mock actors, never the real `Live*` classes against a network layer — SDK-signature correctness is verified by manual code review instead. The most consequential instance is `LiveWorkoutRepository.saveWorkout`, whose real control flow (including the just-fixed critical duplicate-write bug) has no regression-test protection at the `Live` level; a future refactor could silently reintroduce it with all tests still green. Worth a follow-up investment in lightweight HTTP-layer test doubles if this repository keeps growing.
- **`getRecentExerciseOptions` doesn't fall back to `inferMuscleGroupFromName()` for a blank `muscle_group`.** Web falls back to a name-based heuristic; that heuristic isn't ported to Swift yet, so the Swift version falls back to an empty string instead. Documented inline; wire it in if/when that heuristic gets ported.
- **`ActiveWorkoutViewModel.resolveEntry` never resolves `.quickStart`.** Web shows an interstitial "Quick Start" sheet when there's no active draft and no deep link, gated on a user preference (`profile.show_start_sheet`). No `ProfileRepository` exists yet in this codebase, so the quick-start decision was scoped out entirely — `LogEntryView` always resolves straight to a blank session in that case. `WorkoutEntryMode.quickStart` remains on the enum for a future caller to assign directly once a profile-preference source exists.
- **`SetRowView`'s copy/remove is swipe-only**, with no visual affordance hinting that swipe is available (`allowsFullSwipe: false`, so it's a two-step swipe-then-tap). Reasonable as a v1 default matching standard iOS list-row conventions, but flagged for a future discoverability/accessibility pass (e.g. a long-press context menu as a secondary affordance) given the one-handed, mid-set usage context.
- **`PlanEditorViewModel.pendingDecision`** (the mid-session "Update Plan / This Session Only / Cancel" 3-way prompt, triggered when an exercise is added while a plan is loaded) **has no UI wired up anywhere.** `handleAddedExerciseWhilePlanLoaded`/`resolvePendingDecision` exist and are unit-tested, but nothing in `ActiveWorkoutView` currently calls the trigger, since inline plan-loading itself isn't wired into the active session flow yet. Whoever adds that wiring needs to also build the `.confirmationDialog` for this state.
- **`ExercisePickerView`'s My Plans "Edit" opens `PlanEditorView` as sheet-over-sheet** (a second full-screen modal stacked on the already-presented picker), since the picker's own `NavigationStack` can't cleanly push to a screen that needs to outlive the picker's presentation lifecycle. Judged an acceptable, pragmatic tradeoff during code review — SwiftUI has no clean alternative here without a larger nav-architecture change — but flagged for a UX pass later (e.g. a distinguishable nav bar tint so the second layer reads as "two deep" rather than "did my tap not register," or a lighter-weight inline edit affordance instead of a full second screen).
- **`FinishSheetView`'s `touchedExerciseNames` doesn't dedupe** before passing to `PersonalRecordRepository.countNewPRs`. Likely harmless if the repository's `IN`-style filter tolerates duplicate names (it should), but this was never explicitly confirmed — worth a one-line check or dedupe if it turns out to matter.
