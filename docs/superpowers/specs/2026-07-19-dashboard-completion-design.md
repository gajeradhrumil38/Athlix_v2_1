...
### Gap #2: Body-weight-relative muscle load — real per-exercise volume, not just a division

**Revised after discovering a deeper gap during planning**: web's `relativeLoad` isn't raw-load-divided-by-bodyweight — it's real per-exercise training volume (`weight × reps × sets`) divided by bodyweight, computed per exercise then aggregated per muscle slug (`src/pages/Home.tsx:301-327`). The current Swift Dashboard doesn't fetch exercise-level data at all: `DashboardViewModel.muscleLoadBySlug` only has each `Workout`'s coarse `muscleGroups: [String]?` array (workout-level, not exercise-level) and weights every group equally via `ExerciseMuscleMapper.profile(forExerciseName: "", fallbackMuscleGroup: group)` — the empty exercise name means it's not doing real name-based muscle mapping at all, just a fallback lookup. Per explicit approval, this gets the real fix, not a superficial division:

- **New `WorkoutRepository.fetchExercisesForWorkouts(userId: String, workoutIds: [String]) async throws -> [ExerciseSet]`**: a single batched query (`workout_id IN (...)`, chunked in groups of 400 matching `ExerciseLibraryRepository`'s established batching pattern) fetching real exercise rows for every workout already in the Date Navigator's range — far more efficient than N individual `fetchWorkoutExercises` calls, and no separate ownership check needed since the workout ids being queried already came from an authenticated, user-scoped fetch.
- `DashboardViewModel` fetches exercises for the current `workouts` range via this new method, then computes real per-exercise volume (`weight × reps × sets`, mirroring `toDisplayExerciseWeight`-adjusted `exerciseLoad` from web — unit conversion via the existing `WeightUnit.convert`) and real muscle-slug targeting via `ExerciseMuscleMapper.profile(forExerciseName: exercise.name, fallbackMuscleGroup: exercise.muscleGroup)` — using the REAL exercise name this time, matching web's actual per-exercise mapping instead of the current fallback-only approximation. This is a genuine improvement to Muscle Map/Radar accuracy independent of the relative-load feature itself.
- `DashboardViewModel` also fetches the user's `Profile` once (via the new `ProfileRepository`) alongside its existing widget data.
